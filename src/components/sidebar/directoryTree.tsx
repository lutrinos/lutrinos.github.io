...
export default function DirectoryTree({ tree }) {
    ...
    function autoExpand(nodes, parentPath) {
            ...
            <div class="flex flex-col">
                {node.type === "folder" ? (
                    <>
                        ...
                        {expandedFolders[currentPathStr] && node.children && (
                            <div class="flex flex-col pl-3 ml-2 border-l border-(--line-divider)">
                                {node.children.map(child => renderNode(child, currentPathStr, depth + 1))}
                            </div>
                        )}
                    </>
                ) : (
