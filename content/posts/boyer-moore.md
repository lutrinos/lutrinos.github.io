...
Cet algorithme fonctionne en deux parties, la contruction de la table de décalage dans un premier temps puis la recherche en elle-même.

On utilisera comme exemple la recherche du mot _abracadabra_ dans le mot _abrabracadabradabra_.

### La table de décalage

La table de décalage consiste à connaître, pour chaque caractère `c` et chaque indice `i` du motif, la position du premier caractère `c` à gauche de la position `i` dans le motif (en partant de la droite, c'est-à-dire du caractère à la positon `i`).
...
### La recherche
Comme dans l'algorithme naïf, on va partir de l'indice `i = 0` dans le texte et superposer les chaînes de caractère. Sauf que l'on va ensuite commencer à comparer les caractères à partir de la fin du motif !
...
- Si il y a un `d` plus tôt dans le motif, on le place  devant le texte pour que les deux même caractères soient confondus. Formellement, on déplace le motif de `j - k` caractères vers la droite, avec `j` la position du dernier caractère comparé dans le motif (ici `j = 9`), et `k` celle du premier `d` (caractère du texte différent) directement à sa gauche (ici `k = 6`). On remarque `k` est le nombre de la table de décalage correspondant au caractère `d` et à la position `j`.

- S'il n'existe pas, alors on ne peut pas avancer le motif pour que ce caractère du texte soit superposé avec un caractère identique du motif. On doit donc décaler le motif jusqu'à ce qu'il j'ait passé.
...
On décale ensuite le motif vers la droite de un caractère pour chercher d'autres occurences, et on recommence l'algorithme.

![Quatrième comparaison](/images/boyer-moore/image6.png)

Les caractères sont différents, donc on redécale comme avant.
...
Tout d'abord, nous devont connaître la taille du texte et du motif.

Il est possible d'utiliser la fonction `strlen` en important `string.h`, mais on le peut le faire nous-même : la fin d'une séquence de caractère est marquée par un `0` à la fin.
...
On peut donc à présent construire la table de décalage avec la fonction `table` prenant en entrée le tableau à remplir, du texte et la taille de la chaîne de caractère, renvoyant la table sous forme d'un tableau bidimensionnel. Elle est de complexité $O(n)$ avec $n$ la longueur de la chaîne du motif, même si le facteur 256 est grand.
...
On applique ensuite l'algorithme, donc le fonctionnement a été décrit précédemment.
...
[^1]: Dans la pratique, une seconde table de décalage est utilisée pour accélerer encore un peu l'algorithme.4